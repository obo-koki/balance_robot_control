...
int BaseRobotControl::pi;
int BaseRobotControl::count_R;
int BaseRobotControl::count_L;

BaseRobotControl::BaseRobotControl(ros::NodeHandle nh){

    //ROS
    node_handle_ = nh;
    odom_pub_ = nh.advertise<nav_msgs::Odometry>("/odom",5);
    vel_sub_ = nh.subscribe("/cmd_vel", 10, &BaseRobotControl::cmd_vel_callback, this);
    imu_sub_ = nh.subscribe("/imu/data", 10, &BaseRobotControl::imu_callback, this);
    process_timer_ = nh.createWallTimer(ros::WallDuration(PROCESS_PERIOD),&BaseRobotControl::timer_callback,this);

    // For PID debug
    vel_pub_R_ = nh.advertise<geometry_msgs::Twist>("/motor_vel_R",5);
    vel_pub_L_ = nh.advertise<geometry_msgs::Twist>("/motor_vel_L",5);
    PID_sub_R_ = nh.subscribe("/PID_R",10,&BaseRobotControl::PID_R_callback,this);
    PID_sub_L_ = nh.subscribe("/PID_L",10,&BaseRobotControl::PID_L_callback,this);
...
void BaseRobotControl::encoder_count_R_A(){
...
void BaseRobotControl::encoder_count_R_A(int, unsigned int, unsigned int, unsigned int){
...
void BaseRobotControl::encoder_count_R_B(){
    if (digitalRead(EN_R_A) != digitalRead(EN_R_B)){
        count_R --;
    }else{
        count_R ++;
...
void BaseRobotControl::encoder_count_R_B(int, unsigned int, unsigned int, unsigned int){
    if (gpio_read(pi, EN_R_A) != gpio_read(pi, EN_R_B)){
        count_R --;
    }else{
        count_R ++;
...
void BaseRobotControl::encoder_count_L_A(){
...
void BaseRobotControl::encoder_count_L_A(int, unsigned int, unsigned int, unsigned int){
...
void BaseRobotControl::encoder_count_L_B(){
    if (digitalRead(EN_L_A) != digitalRead(EN_L_B)){
        count_L --;
    }else{
        count_L ++;
...
void BaseRobotControl::encoder_count_L_B(int, unsigned int, unsigned int, unsigned int){
    if (gpio_read(pi, EN_L_A) != gpio_read(pi, EN_L_B)){
        count_L --;
    }else{
        count_L ++;
...
void BaseRobotControl::cmd_vel_callback(const geometry_msgs::Twist::ConstPtr &vel){
    std::lock_guard<std::mutex> lock(m);
    target_vel_R = vel->linear.x + vel->angular.z * WHEEL_DIST;
    target_vel_L = vel->linear.x - vel->angular.z * WHEEL_DIST;
}

void BaseRobotControl::imu_callback(const sensor_msgs::Imu::ConstPtr &imu){
    std::lock_guard<std::mutex> lock(m);
    //robot pose contorol
}

void BaseRobotControl::PID_R_callback(const std_msgs::Float32MultiArray &msg){
...
void BaseRobotControl::PID_L_callback(const std_msgs::Float32MultiArray &msg){
    int num = msg.data.size();
    if (num !=3){
...
float BaseRobotControl::calc_angle_output(int _count){
...
void BaseRobotControl::calc_odom(){
...
void BaseRobotControl::timer_callback(const ros::WallTimerEvent &e){
    std::lock_guard<std::mutex> lock(m);
    //Timer debug
...
void BaseRobotControl::motor_stop(){
    gpio_write(pi, MOTOR_DRIVER_EN, PI_HIGH);
    printf("Force motor stop\n");
    exit(1);
}

void BaseRobotControl::motor_control(){
    bool dir_R, dir_L;
...
void BaseRobotControl::main_loop(){
    //Motor start
